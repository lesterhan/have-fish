...
function row(
  over: Partial<AccountCoverageStatus> & { accountId: string },
): AccountCoverageStatus {
  return {
    state: 'current',
    coveredThrough: '2026-09-04',
    dormant: false,
    ...over,
  }
}
...
const unset = (id: string) =>
  row({ accountId: id, state: 'unset', coveredThrough: null })
...
describe('completeness', () => {
  ...
  it('treats a behind account with no leading edge as unknown', () => {
    const result = completeness([
      row({ accountId: 'a', state: 'behind', coveredThrough: null }),
    ])
  ...
  it('reports no contributors when every account is dormant', () => {
    ...
    expect(completeness([dormant])).toEqual({
      through: null,
      unknown: 0,
      contributors: 0,
    })
...
describe('coverageFor', () => {
  ...
  it('drops ids the coverage payload does not carry', () => {
    expect(
      coverageFor(byId, ['a', 'not-tracked']).map((r) => r.accountId),
    ).toEqual(['a'])
  })
  ...
  it('feeds completeness so a tile dates itself from its own contributors', () => {
    ...
    expect(completeness(coverageFor(byId, ['a', 'b'])).through).toBe(
      '2026-06-21',
    )
  })
})
...
describe('formatCompletenessDate', () => {
  ...
  it('keeps the year once the date is from another one', () => {
    expect(formatCompletenessDate('2025-06-21', '2026-09-04')).toBe(
      'Jun 21, 2025',
    )
  })
})
...
describe('completenessNote', () => {
  ...
  const note = (rows: AccountCoverageStatus[]) =>
    completenessNote(completeness(rows), TODAY)
  ...
  it('qualifies the date with the unknown rather than replacing it', () => {
    ...
    expect(result?.text).toBe(
      'complete through Jun 21, 1 account has no starting line',
    )
    expect(result?.current).toBe(false)
  ...
  it('drops the date clause when there is no date to give', () => {
    expect(note([current('a'), unset('b')])?.text).toBe(
...
describe('statusNote', () => {
  const TODAY = '2026-09-04'
  const note = (rows: AccountCoverageStatus[]) =>
    statusNote(completeness(rows), TODAY)
  ...
  it('names its subject, unlike the line under a tile', () => {
    expect(note([behind('a', '2026-06-21')])?.text).toBe(
      'Ledger complete through Jun 21',
    )
  })
  ...
  it('takes its date from the stalest live account', () => {
    expect(
      note([behind('a', '2026-08-15'), behind('b', '2026-04-23')])?.text,
    ).toBe('Ledger complete through Apr 23')
  })
  ...
  it('is not held back by a dormant account', () => {
    ...
    expect(note([current('a'), dormant])?.text).toBe(
      'Ledger complete through today',
    )
  })
  ...
  it('carries the unrecorded accounts alongside the date', () => {
    ...
    expect(result?.text).toBe(
      'Ledger complete through Jun 21 — 1 account has no starting line',
    )
    expect(result?.current).toBe(false)
  })
  ...
  it('agrees with the tile line on which date is quoted', () => {
    ...
    expect(completenessNote(completeness(rows), TODAY)?.text).toContain(
      'Jun 21',
    )
  })
})
