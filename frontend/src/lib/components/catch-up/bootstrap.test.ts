...
describe('describeProposal', () => {
  ...
  it('describes a single day', () => {
    expect(describeProposal(proposal({ fromDate: '2025-06-30' }))).toBe(
      'Marks 2025-06-30 as covered',
    )
  })
  ...
  it('says so when there is no history behind the row', () => {
    expect(describeProposal(proposal({ noHistory: true }))).toBe(
      'No transactions yet — marks today as covered',
    )
  })
})
